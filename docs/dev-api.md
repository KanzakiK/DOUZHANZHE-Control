...
### POST /api/gpu/set
GPU 核心/显存频率控制（nvidia-smi 子进程）。Body: { action: string, value?: int, min?: int, max?: int }

| action | 功能 | 命令 |
|--------|------|------|
| limit-max | 核心上限限制 | --lock-gpu-clocks=0,value |
| lock-exact | 核心锁定 | --lock-gpu-clocks=value |
| limit-memory | 显存上限限制 | --lock-memory-clocks=0,value |
| lock-memory-clocks | 显存锁定 | --lock-memory-clocks=value,value |
| 
eset-clocks | 重置核心频率 | --reset-gpu-clocks |

| 
eset-memory-clocks | 重置显存频率 | --reset-memory-clocks |
