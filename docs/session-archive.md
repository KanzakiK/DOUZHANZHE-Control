...
| 日期 | 会话主题 |
|:-----|:--------|
| 2026-06-07 | [风扇信息 UI 优化 — 删除 RPM / max 后缀](#2026-06-07-风扇信息-UI-优化) |
...
- **合并**：删除 useControlState.js 私有 MODE_PRESETS 副本，统一 import uxtuAdapter.js 导出版
...
## 2026-06-07 (风扇信息 UI 优化 — 删除 RPM / max 后缀)
- **修改**：TelemetryPanel.jsx 风扇信息卡片删除大风扇/小风扇转速后的 `/ max` 后缀显示（`3000 RPM / 4400` → `3000 RPM`）
- **验证**：真机页面确认显示正确，`_verify_write.py` 后验清洗 CLEAN
...
## 2026-06-06 (每模式独立参数记忆 localStorage + 恢复预设)
- 新增：每模式独立参数记忆；匹配模式切换保存旧值+加载新值
- 修复：模式按钮不再覆盖 uxtuParams，交由 useControlState prevModeRef effect 统一负责
- 修复：切换时先保存旧模式参数再加载新模式记忆，解决风扇落回问题
- 优化：参数变化时实时保存到 localStorage，避免切换时丢失
---
