...
- **各模式独立配置持久化**: 每个性能模式（安静/均衡/野兽/斗战）的自定义参数现在独立保存到后端配置文件，不再依赖浏览器 localStorage。重装系统、清除浏览器数据不会丢失设置，每个模式的功耗、频率、风扇、超频等参数各自记忆。**注意**：切换模式时界面会暂时锁定，需等待参数下发完成（约 3-5 秒）
...
- **手动风扇转速不稳定**: 手动设置的风扇转速容易被 EC 固件覆盖。现在改用与自定义风扇曲线相同的写入方式（WMI + EC 寄存器直写），并增加 ParameterGuard 每 60 秒守护。**注意：EC 会根据当前温度墙和功耗设置自动调节风扇，实际转速可能低于设定值。例如斗战模式下需要将温度墙设为 95°C、长时功耗 90W、短时功耗 120W 并开启 CPU 压力测试，风扇才能达到目标转速 4300 RPM。低温度墙或低功耗设置下，EC 会主动限制风扇转速**
