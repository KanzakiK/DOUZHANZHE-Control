...
- **手动风扇转速不稳定**: 手动设置的风扇转速容易被 EC 固件覆盖。现在改用与自定义风扇曲线相同的写入方式（WMI + EC 寄存器直写），并增加 ParameterGuard 每 60 秒守护。注意：EC 会根据当前功耗/TDP 设置自动调节风扇，如果 TDP 设置远低于模式默认值，实际转速可能仍会低于设定值
