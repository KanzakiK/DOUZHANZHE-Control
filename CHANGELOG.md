...
- **启动日志显示硬件信息**: 启动时自动记录版本号、GPU 型号、驱动版本、显卡模式等关键信息，方便排查问题时快速定位环境

### 改进

- **问题排查更容易**: GPU 频率调节、超频偏移、温度墙等操作现在都有详细日志记录，遇到异常时可以精确定位是哪一步出了问题
- **安装包体积缩小**: 移除了多余的语言资源文件，安装包从约 5.5 MB 缩小到约 5.1 MB
