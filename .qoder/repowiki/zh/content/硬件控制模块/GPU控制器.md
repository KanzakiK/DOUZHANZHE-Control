...
**图表来源**
- [Program.cs:14-120](file://server/api/Program.cs#L14-L120)
...
- 硬件驱动不可用
  - 现象：温度/风扇读取为0或异常
  - 排查：确认inpoutx64驱动加载、管理员权限、BIOS设置
  - 参考：HAL健康检查与驱动降级逻辑
...
## 结论
本GPU控制器通过清晰的分层架构实现了对多平台GPU与CPU电源管理的统一控制。后端以HAL为核心，向上提供稳定的API与WebSocket遥测，向下封装nvidia-smi、ryzenadj与WMI等外部工具与固件接口。前端通过直观的UI与模式预设简化了用户的操作体验。系统具备良好的扩展性与容错能力，适合在多种笔记本与台式机平台上部署与使用。
