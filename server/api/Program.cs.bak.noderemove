...
// ---- Reverse proxy HttpClient (legacy Node.js endpoints) ----
var _proxyClient = new HttpClient { Timeout = TimeSpan.FromSeconds(10) };
...
// ---- Reverse proxy to Node.js :3099 for unhandled /api/* ----
app.Use(async (context, next) =>
{
        ...
        && context.Request.Path.StartsWithSegments("/api", StringComparison.OrdinalIgnoreCase))
    {
        try
        {
            var target = $"http://localhost:3099{context.Request.Path}{context.Request.QueryString}";
            var req = new HttpRequestMessage(new HttpMethod(context.Request.Method), target);
            if (context.Request.Method != "GET" && context.Request.Method != "HEAD" && context.Request.Body != null)
            {
            ...
            var resp = await _proxyClient.SendAsync(req, HttpCompletionOption.ResponseHeadersRead);
        ...
        catch (Exception ex)
        {
            ...
            await context.Response.WriteAsync($"{{\"ok\":false,\"error\":\"proxy: {ex.Message}\"}}");
        }
    }
});
