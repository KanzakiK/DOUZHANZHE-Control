...
app.MapGet("/api/background-opts", () =>
{
    try
    {
        var cfg = JsonRead<Dictionary<string, object?>>("background.json", new Dictionary<string, object?>());
        return Results.Json(new
        {
            enabled = cfg.ContainsKey("enabled") && Convert.ToBoolean(cfg["enabled"]),
            opacity = cfg.ContainsKey("opacity") ? Convert.ToInt32(cfg["opacity"]) : 50,
            maskColor = cfg.ContainsKey("maskColor") ? cfg["maskColor"]?.ToString() : "black",
            hasImage = File.Exists(bgImagePath),
        });
    }
    catch { return Results.Json(new { enabled = false, opacity = 50, maskColor = "black", hasImage = false }); }
});

app.MapPost("/api/background-opts", (HttpContext ctx) =>
{
    try
    {
        using var reader = new StreamReader(ctx.Request.Body);
        var body = JsonSerializer.Deserialize<Dictionary<string, object?>>(reader.ReadToEndAsync().Result);
        if (body != null) JsonWrite("background.json", body);
        return Results.Json(new { ok = true });
    }
    catch (Exception ex) { return Results.Json(new { ok = false, error = ex.Message }); }
});
...
{
    if (!File.Exists(bgImagePath)) return Results.NotFound();
    ctx.Response.Headers["Cache-Control"] = "no-store";
    return Results.File(File.OpenRead(bgImagePath), "image/png", enableRangeProcessing: false);
});
...
app.MapPost("/api/background", async (HttpContext ctx) =>
{
    try
    {
        ...
        {
            var dataUrl = imageEl.GetString();
            if (string.IsNullOrEmpty(dataUrl)) return Results.Json(new { ok = false, error = "图片数据为空" });
            var commaIdx = dataUrl.IndexOf(',');
            if (commaIdx < 0) return Results.Json(new { ok = false, error = "无效的 base64 格式" });
            var base64 = dataUrl.Substring(commaIdx + 1);
            var bytes = Convert.FromBase64String(base64);
            File.WriteAllBytes(bgImagePath, bytes);
            var cfg = JsonRead<Dictionary<string, object?>>("background.json", new Dictionary<string, object?>());
            cfg["hasImage"] = true;
            JsonWrite("background.json", cfg);
            return Results.Json(new { ok = true });
        }
        return Results.Json(new { ok = false, error = "缺少 image 字段" });
    }
    catch (Exception ex) { return Results.Json(new { ok = false, error = ex.Message }); }
});
...
app.MapDelete("/api/background", () =>
{
    try
    {
        if (File.Exists(bgImagePath)) File.Delete(bgImagePath);
        var cfg = JsonRead<Dictionary<string, object?>>("background.json", new Dictionary<string, object?>());
        cfg["hasImage"] = false;
        JsonWrite("background.json", cfg);
        return Results.Json(new { ok = true });
